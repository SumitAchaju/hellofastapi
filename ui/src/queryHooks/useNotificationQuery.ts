  ...
  return useInfiniteQuery({
    ...
    queryFn: async ({ pageParam }): Promise<notificationType[]> => {
      const res = await api.get(
        notificationUrl.getNotification(LIMIT, pageParam)
      );
      return res.data;
    },
    initialPageParam: 0,
...
export function useNotificationMutation() {
  ...
  const notificationStatusChange = useMutation({
    ...
    mutationFn: async ({ id, data }: notificationMutationType) => {
      const res = await api.patch(notificationUrl.markOrChangeStatus(id), data);
      return res.data;
    },
  ...
  const notificationMarkAllRead = useMutation({
    ...
    mutationFn: async () => {
      const res = await api.patch(notificationUrl.markAllRead);
      return res.data;
    },
    onMutate: async () => {
      await queryClient.cancelQueries({ queryKey: [KEY] });
  ...
  const notificationDelete = useMutation({
    ...
    mutationFn: async (id: number) => {
      const res = await api.delete(notificationUrl.notificationDelete(id));
      return res.data;
    },
  ...
  const notificationDeleteAll = useMutation({
    ...
    mutationFn: async () => {
      const res = await api.delete(notificationUrl.deleteAll);
      return res.data;
    },
    onSettled: () => {
      queryClient.invalidateQueries({ queryKey: [KEY] });
